...
var chargeStatistiquesPresence = function(e) {
	...
	var datafields = [ {
		...
		name : 'nbPresenceConfirme',
		type : 'int'
	}, {
		name : 'nbAbsence',
		type : 'int'
	}, {
		name : 'nbAbsenceConfirme',
		type : 'int'
	}, {
		name : 'nbInconnu',
		type : 'int'
	...
	var columns = [ {
		...
		text : 'dont confirmée',
		datafield : 'nbAbsenceConfirme',
		width : '15%'
	}, {
		text : 'Presence',
		datafield : 'nbPresence',
		width : '15%'
	}, {
		text : 'dont confirmée',
		datafield : 'nbPresenceConfirme',
		width : '15%'
	}, {
		text : 'Inconnu',
		datafield : 'nbInconnu',
...
var chargeListeEtapes = function() {
	...
	req.fail(function(jqXHR) {
		ajaxFailFunctionToDisplayWarn(jqXHR, "le chargement de la liste des étapes");
	});
}
...
var chargeStatistiques = function() {
	...
	req.success(function(dataString) {
		...
		$("#repartitionsFoyer").jqxGrid({
			source : dataAdapterFoyer,
			sortable : true,
			altrows : true,
			autoheight : true,
			autowidth : true,
			columns : [ {
				text : 'Répartition par foyer',
				datafield : 'clef'
			}, {
				text : 'Nombre d\'invités',
				datafield : 'valeur'
		...
		$("#repartitionsInviteParEtape").jqxGrid({
			source : dataAdapterInviteEtape,
			sortable : true,
			altrows : true,
			autoheight : true,
			autowidth : true,
			columns : [ {
				text : 'Répartition invités par etape',
				datafield : 'clef'
			}, {
				text : 'Nombre d\'invités',
				datafield : 'valeur'
		...
		$("#repartitionsFoyerParEtape").jqxGrid({
			source : dataAdapterFoyerEtape,
			sortable : true,
			altrows : true,
			autoheight : true,
			autowidth : true,
			columns : [ {
				text : 'Répartition foyers par etape',
				datafield : 'clef'
			}, {
	...
	req.fail(function(jqXHR) {
		ajaxFailFunctionToDisplayWarn(jqXHR, "le chargement des statistiques");
	});
};
